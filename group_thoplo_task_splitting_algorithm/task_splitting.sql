...
*************************************/
-- magic function that takes in building geometry and bounding geometry
-- and returns 
,hulls AS(
  select Ivan_Hull(ST_Collect(clb.geom), clb.clusterid, tsk.geom) as geom
  from clusteredbuildings clb
  LEFT JOIN taskpolygons ON clb.polyid = tsk.polyid
)
